...
import           GHCJS.DOM.Types            (MonadJSM, castTo, liftJSM)
...
radioGroup :: forall t m a.(MonadWidget t m, Eq a, MonadJSM m)
  -- MonadFail m, 
           => Dynamic t Text
  ...
  where
    ...
    handleOne _ dynV dynChecked = do
      ...
      el "tr" $ el "td" $ el "label" $  do
        ...
        --NOTE modified
        do
            e' <- castTo_ HTMLInputElement $ _element_raw b
            case e' of
              Nothing -> do
                  error "[radioGroup] assertion failed: element is an HTMLInputElement" --TODO
              Just e  -> do
                  _ <- performEvent $ do
                      (setChecked_ e) <$> updated dynChecked
                  return()

        return (Click `domEvent` b, f)

        where
        --NOTE ^ lift the functions into methods to loosen `MonadJSM IO` into `MonadJSM m`
